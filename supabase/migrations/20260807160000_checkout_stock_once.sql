-- Fix double stock drop: dedupe cart/order lines, inline stock in checkout, clear cart in RPC.

-- 1) Merge duplicate cart lines (same variant twice in one cart → summed qty once).
with ranked as (
  select
    id,
    cart_id,
    ...
    sum(quantity) over (partition by cart_id, variant_id) as total_qty
...
using public.cart_items dup
where ci.cart_id = dup.cart_id
  and ci.variant_id = dup.variant_id
...
-- 2) Merge duplicate order lines on past orders (same variant twice → one line).
with ranked as (
  select
    id,
    order_id,
    ...
    quantity,
    row_number() over (
      partition by order_id, variant_id
      order by created_at asc, id asc
    ) as rn,
    sum(quantity) over (partition by order_id, variant_id) as total_qty
...
using public.order_items dup
where oi.order_id = dup.order_id
  and oi.variant_id = dup.variant_id
...
-- 3) Checkout: inline stock once, clear cart inside same transaction.
...
begin
  ...
  -- Pre-check stock (aggregated per variant from payload).
  for r in
    select
