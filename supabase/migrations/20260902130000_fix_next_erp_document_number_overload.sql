-- Resolve ambiguous next_erp_document_number(text) vs (text, uuid DEFAULT NULL).
-- Purchase bill finalize → journal posting was failing with:
--   function public.next_erp_document_number(unknown) is not unique
...
-- Journal numbers: rely on trg_journal_entries_document_ref (no ambiguous call).
...
BEGIN
  ...
  VALUES (
    '', p_transaction_date, COALESCE(p_description, ''), p_store_id, v_company_id,
...
-- Cash book rows: rely on trg_erp_account_transactions_document_ref.
...
BEGIN
  ...
  LOOP
    ...
    VALUES (
      '', v_line.account_id, v_counter, v_je.store_id,
