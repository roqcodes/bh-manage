-- Convert online / sales orders to ERP invoices for accounting (revenue, VAT, AR).
-- Skips stock deduction when order inventory already committed.
...
BEGIN
  ...
  -- Prefer order header totals when present (matches what user saw on order).
